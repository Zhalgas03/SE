...
trip_api = Blueprint('trip_api', __name__, url_prefix='/api')

UPLOAD_FOLDER = os.path.join(os.path.dirname(__file__), '..', 'uploads')
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

@trip_api.route('/save_trip', methods=['POST'])
def save_trip():
...
@trip_api.route('/get_trip/<int:trip_id>', methods=['GET'])
    ...
    if rule:
        if total_votes >= rule.min_votes:
            ...
            if ratio >= rule.threshold:
                ...
                status = "rejected"
    download_url = url_for('trip_api.download_pdf', trip_id=trip.id, _external=True)
    return jsonify({
        "trip_id": trip.id,
        "title": trip.title,
...
@trip_api.route('/download/<int:trip_id>', methods=['GET'])
