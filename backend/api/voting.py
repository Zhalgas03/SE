...
voting_api = Blueprint('voting_api', __name__, url_prefix='/api')

@voting_api.route('/vote', methods=['POST'])
def vote():
    ...
    if not all([trip_id, session_token]) or value not in [1, -1]:
        return jsonify(success=False, message="trip_id, session_token, and value are required (value must be 1 or -1)"), 400
...
@voting_api.route('/vote_status/<int:trip_id>', methods=['GET'])
