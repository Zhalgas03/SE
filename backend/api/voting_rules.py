...
@voting_bp.route("/enable", methods=["POST"])
@jwt_required()
def enable_voting():
    try:
        data = request.get_json()
        ...
        if existing:
            voting_link = f"http://localhost:3000/guest-vote/{trip_id}"
            return jsonify(success=True, link=voting_link), 200

        cur.execute("""
            INSERT INTO voting_rules (trip_id, created_by)
            VALUES (%s, %s)
        """, (trip_id, username))
        conn.commit()
        ...
        print("Error in /voting/enable:", e)
